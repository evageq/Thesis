...
создать отдельный диск размеров не менее 10 ГБ.
...
В итоге объем одной транзакции равен (Формула \ref{formula:trans})
\begin{equation}
        \label{formula:trans}
        V_\text{транзакции} = 736 * 2 = 1472 \; \text{бит} \approx 0.002 \; \text{Мбит}
\end{equation}
...
3 транзакции в секунду, что примерно равно 0.006 Мбит/с. Это значит, что на интерфейс коммутатора только от 
службы времени будет поступать как минимум 0.006 Мбит/с трафика или с учетом расчета количества пакетов – 6.5 пакета в секунду.


Следующим представлен расчет нагрузки для службы DNS. На Рисунке \ref{fig:wireshark_dns_dump} видно, что 
транзакция состоит из запроса к серверу и его ответа клиенту,
максимальный размер пакета 1048 бит, объем транзакции 2 пакета.
...
В итоге объем одной транзакции равен (Формула \ref{formula:dns_trans})
\begin{equation}
        \label{formula:dns_trans}
        V_\text{транзакции} = 1048 * 2 = 2096 \; \text{бит} \approx 0.002 \; \text{Мбит}
\end{equation}
...
\textbf{Отдел} & \textbf{Количество АРМ} & \textbf{Количество обращений (транзакций) за рабочий день, шт } & \textbf{Всего пакетов за рабочий день, шт }\\
\hline
Руководство предприятия & 1 & 2400 & 4800 \\
\hline
Коммерческий отдел & 3 & 7200 & 14400 \\
\hline
Бухгалтерия & 10 & 24000 & 48000 \\
\hline
Отдел кадров & 20 & 48000 & 96000 \\
\hline
...
АХО & 2 & 4800 & 9600 \\
\hline
ИТ-департамент & 20 & 48000 & 96000 \\
\hline
Профильные отделы & 45 & 108000 & 216000 \\
\hline
Итого & - & 1346400 & 2692800 \\
\hline
\end{tabular}
\end{table}

В итоге за 8-часовой рабочий день формируется примерно 1346400 транзакций для 
службы DNS, что примерно равно 
46 транзакций в секунду, что примерно равно 0.1 Мбит/с. 
Это значит, что на интерфейс коммутатора только от 
службы времени будет поступать как минимум 0.1 Мбит/с трафика 
или с учетом расчета количества пакетов – 93.5 пакета в секунду.


Далее представлен расчет нагрзуки для веб службы. На Рисунке 
\ref{fig:wireshark_web_dump} видно, что транзакция состоит из запроса к DNS,
TCP handshake, обработки HTTP запроса, закрытия TCP соединения, максимальный
пакет транзакции 46880 бит, всего пакетов в транзакции 16. На рисунке \ref{fig:wireshark_web_stat_dump}
изображена полная статистика транзакции, из чего видно, что объем транзакции равен 68544 бит.
\begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{web_wireshark.png}
        \caption{Анализ трафика веб службы}
        \label{fig:wireshark_web_dump}
\end{figure}
\begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{web_wireshark_stat.png}
        \caption{Анализ трафика веб службы}
        \label{fig:wireshark_web_stat_dump}
\end{figure}
...
17 транзакций в секунду, что примерно равно 1 Мбит/с. 
Это значит, что на интерфейс коммутатора только от 
веб службы будет поступать как минимум 1 Мбит/с трафика 
или с учетом расчета количества пакетов – 272 пакета в секунду.
...
из 258 пакетов и генерирует 86016 бит трафика, максимальный размер пакета 20448 бит.
...
\begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{freeipa_wireshark_stats.png}
...
2 транзакций в секунду, что примерно равно 0.2 Мбит/с. 
Это значит, что на интерфейс коммутатора только от 
службы управления пользователями будет поступать как минимум 0.2 Мбит/с трафика 
или с учетом расчета количества пакетов – 582 пакета в секунду.
...
количество пакетов в транзакции 591, объем транзакции равен 3 Мбит.
...
\begin{figure}[H]
        \centering
        \includegraphics[scale=0.5]{iscsi_stats_wireshark.png}
        \caption{Детали транзакции файловой службы}
        \label{fig:wireshark_iscsi_stats_dump}
\end{figure}

Количество обращений к веб службе представлено в Таблице \ref{table:iscsi_requests}.
\begin{table}[H]
\centering
\small
...
\textbf{Отдел} & \textbf{Количество АРМ} & \textbf{Количество обращений (транзакций) за рабочий день, шт } & \textbf{Всего пакетов за рабочий день, шт }\\
\hline
Руководство предприятия & 1 & 100 & 59100\\
\hline
Коммерческий отдел & 3 & 900 & 53190 \\
\hline
Бухгалтерия & 10 & 10000 & 5910000 \\
\hline
Отдел кадров & 20 & 1000 & 591000 \\
\hline
Отдел закупок & 30 & 3000 & 1773000\\
\hline
Отдел продаж & 55 & 10000 & 5910000 \\
\hline
АХО & 2 & 20 & 11820 \\
\hline
ИТ-департамент & 20 & 5000 &  2955000\\
\hline
Профильные отделы & 45 & 5000 & 2955000 \\
\hline
Итого & - & 35020 &  20206290 \\
\hline
\end{tabular}
\end{table}

В итоге за 8-часовой рабочий день формируется примерно 35020 транзакций для 
файловой службы, что примерно равно 
1.5 транзакций в секунду, что примерно равно 4.5 Мбит/с. 
Это значит, что на интерфейс коммутатора только от 
файловой службы будет поступать как минимум 4.5 Мбит/с трафика 
или с учетом расчета количества пакетов – 702 пакета в секунду.

Таким образом медианное значение максимального размера пакета равно 20448, сумма всех пакетов равна 1655.
На основе входных данных примера минимальная скорость канала будет равна примерно 40 Мбит/с(Формула \ref{}).
\begin{equation}
        V_\text{канала} \ge \frac{20448 * 1655}{0.8}
\end{equation}
...
\begin{figure}[H]
        \centering
        \includegraphics[scale=0.6]{services/web/index.png}
